...
build_libretro_mednafen()
{
   ...
   if [ -d "$MEDNAFEN_DIR_NAME" ]; then
      ...
      make core=pce-fast platform=$FORMAT_COMPILER_TARGET -j$JOBS clean || die "Failed to clean mednafen/${core}"
      make core=pce-fast platform=$FORMAT_COMPILER_TARGET -j$JOBS || die "Failed to build mednafen/${core}"
...
build_libretro_s9x_next()
{
   ...
   if [ -d "$S9X_NEXT_DIR_NAME" ]; then
      ...
      make -f Makefile.libretro platform=$FORMAT_COMPILER_TARGET -j$JOBS clean || die "Failed to build SNES9x-Next"
      make -f Makefile.libretro platform=$FORMAT_COMPILER_TARGET -j$JOBS || die "Failed to build SNES9x-Next"
...
build_libretro_fba()
{
   ...
   if [ -d "$FBA_DIR_NAME" ]; then
      ...
      cp libretro_ngc.a $RARCH_DIST_DIR/fba_cores_cps2_libretro$FORMAT.a
      cd ../../
      ...
      cp libretro_ngc.a $RARCH_DIST_DIR/fba_cores_neo_libretro$FORMAT.a
   else
...
build_libretro_vba()
{
   ...
   if [ -d "$VBA_NEXT_DIR_NAME" ]; then
      ...
      make -f Makefile.libretro platform=$FORMAT_COMPILER_TARGET -j$JOBS clean || die "Failed to clean VBA-Next"
      make -f Makefile.libretro platform=$FORMAT_COMPILER_TARGET -j$JOBS || die "Failed to build VBA-Next"
...
build_libretro_gambatte()
{
   ...
   if [ -d "$GAMBATTE_DIR_NAME" ]; then
      ...
      make -f Makefile.libretro platform=$FORMAT_COMPILER_TARGET -j$JOBS clean || die "Failed to clean Gambatte"
      make -f Makefile.libretro platform=$FORMAT_COMPILER_TARGET -j$JOBS || die "Failed to build Gambatte"
...
build_libretro_prboom()
{
   ...
   if [ -d "$PRBOOM_DIR_NAME" ]; then
      ...
      cd $PRBOOM_DIR_NAME
      make platform=$FORMAT_COMPILER_TARGET -j$JOBS clean || die "Failed to clean PRBoom"
      make platform=$FORMAT_COMPILER_TARGET -j$JOBS || die "Failed to build PRBoom"
